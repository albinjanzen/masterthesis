...
filename = 'nordpool_sweden.csv'
path_to_raw = '../../data/Raw/nordpool/Sweden/'
...
for i in years:
    ...
    for f in files:
        ...
        dfRaw = dfRaw.drop(dfRaw[dfRaw['Data type'] == 'ST'].index)
        dfRaw = dfRaw.drop(dfRaw[dfRaw['Data type'] == 'BE'].index)
        dfRaw = dfRaw.drop(dfRaw[dfRaw['Data type'] == 'AL'].index)
...
for col in date_df.columns:
    date_df[col] = date_df[col].str.replace(',', '.').astype(float)
