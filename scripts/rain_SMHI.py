...
stations = [52350, 84050, 95130, 146050, 189720]
df = pd.DataFrame(pd.date_range(start='1/1/2018',
                                end='12/31/2022', freq='H'), columns=['Date'])
for station in stations:
    response = requests.get(
        'https://opendata-download-metobs.smhi.se/api/version/latest/parameter/7/station/' + str(station) + '/period/corrected-archive/data.csv')
...
for station in stations:
    ...
    temporary.rename(
        columns={'Nederbördsmängd': str(station)+'_rain'}, inplace=True)
